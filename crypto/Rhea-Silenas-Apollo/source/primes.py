...
# Partially (functions fermat_primality_test, square_and_multiply, miller_rabin_primality_test and generate_primes) sourced from https://github.com/NikolaiT/Large-Primes-for-RSA/blob/5a6e82906d87160a4f115acbb950c3ec8334be83/generate_primes.py, licensed under GNU GPLv3, see https://github.com/NikolaiT/Large-Primes-for-RSA/blob/5a6e82906d87160a4f115acbb950c3ec8334be83/LICENSE
...
k = []
while len(pr) < 5:
    p = generate_primes(300, 1)[0]
    q = generate_primes(300, 1)[0]
    if math.gcd((p-1)*(q-1), 5) == 1:
        pr.append(p*q)
        k.append(pow(5,-1,(p-1)*(q-1)))

pks_f = open("public_keys.txt", 'w')
ciphers_f = open("ciphers.txt", 'w')
print(pr)
#generate the public keys
for i in range(5):
    print("Public key " + str(i) + " : " + str(pr[i]) + ", 5")
    pks_f.write("Public key " + str(i) + " = " + str(pr[i]) + ", 5\n")

m =int.from_bytes(b"Trojan{C0m3_0v3r}", "big")
...
for i in range(5):
    c = pow(m, 5, pr[i])
    ciphers.append(c)
    print("Ciphertext " + str(i) + " : " + str(c))
    ciphers_f.write("Ciphertext " + str(i) + " = " + str(c) + "\n")


for i in range(5):
    print("Decoded " + str(i) + " : " + str(pow(ciphers[i], k[i], pr[i])))